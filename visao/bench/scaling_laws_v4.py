#!/usr/bin/env python3
"""
Scaling Laws v4 — Simplified readout + layer norm + L2 + gradient clipping.

Key changes vs v3:
  - Remove consolidation/surprise/Oja (they cause over-consolidation at scale)
  - Add layer normalization on readout input (stabilizes across hidden sizes)
  - Add L2 weight decay (1e-4)
  - Add gradient clipping (max_norm=1.0)
  - 6 hidden sizes from 32 to 1024 (1K to 1M+ params)
  - n_train=2000, epochs=5, 3 seeds
  - Report R² and only accept if R²>0.7
"""
...
def run_experiment(n_hidden, seed):
    ...
    # Simple brain: no consolidation, no surprise, no Oja
    # Uses layer_norm + gradient clipping + L2 via brain.py features
    brain = VisaoBrain(
    ...
    for i in range(len(X_test)):
        pred = brain.forward(X_test[i:i+1])
...
def main():
    ...
    interp = "POSITIVE (good)" if alpha > 0.05 and r_squared > 0.7 else \
             "NEGATIVE (poor)" if alpha < -0.05 and r_squared > 0.7 else \
             "UNCERTAIN (low R²)" if r_squared < 0.7 else "NEUTRAL"
    ...
    if r_squared < 0.7:
        print(f"  ⚠️  WARNING: R² < 0.7 — scaling fit unreliable")

    out = ROOT / 'bench' / 'results_scaling_v4.json'
