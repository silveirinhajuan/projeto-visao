#!/usr/bin/env python3
"""
Scaling Laws v3 — Otimizado.
Range: 32-512 hidden, n_train=1000, 3 seeds, 3 epochs.
"""
...
def run_experiment(n_hidden, seed):
    ...
    brain = VisaoBrain(n_in=1, n_hidden=n_hidden, n_out=1, lr=0.02, sparsity=0.0,
                       consolidation=0.0, surprise_gain=0.0, seed=seed)
    ...
    return {'n_hidden': n_hidden, 'n_params': n_params, 'mse_test': mse_test, 'wall_time': wall_time, 'seed': seed}
...
def main():
    ...
    out = ROOT / 'visao' / 'bench' / 'results_scaling_v3.json'
    out.write_text(json.dumps({
        'version': 'v3',
        'scaling_law': {'alpha': float(alpha), 'alpha_se': float(se_alpha),
                       'alpha_ci_95': [float(ci_lo), float(ci_hi)],
                       'C': float(np.exp(log_C)), 'R_squared': float(r_squared),
                       'interpretation': interp},
        'config': {'n_train': N_TRAIN, 'n_test': N_TEST, 'epochs': EPOCHS, 'seeds': SEEDS},
        'aggregated': agg,
        'raw_results': results,
    }, indent=2))
    print(f"\nSaved to {out}")
